import { useState } from "react"

const ColorTheme = (props: any) => {
  const [theme, setTheme] = useState(0)

  const iconsTheme = [
    {
      name: "system",
      icon: <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-monitor-icon lucide-monitor"><rect width="20" height="14" x="2" y="3" rx="2"/><line x1="8" x2="16" y1="21" y2="21"/><line x1="12" x2="12" y1="17" y2="21"/></svg>
    },
    {
      name: "dark",
      icon: <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-moon-icon lucide-moon"><path d="M20.985 12.486a9 9 0 1 1-9.473-9.472c.405-.022.617.46.402.803a6 6 0 0 0 8.268 8.268c.344-.215.825-.004.803.401"/></svg>
    },
    {
      name: "light",
      icon: <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-sun-icon lucide-sun"><circle cx="12" cy="12" r="4"/><path d="M12 2v2"/><path d="M12 20v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="m17.66 17.66 1.41 1.41"/><path d="M2 12h2"/><path d="M20 12h2"/><path d="m6.34 17.66-1.41 1.41"/><path d="m19.07 4.93-1.41 1.41"/></svg>
    }
  ]

  const handleChangeTheme = () => {
    const newTheme = (theme + 1) % iconsTheme.length
    setTheme(newTheme)

    if (iconsTheme[newTheme].name === "dark") {
      document.documentElement.classList.add("dark")
    } else if (iconsTheme[newTheme].name === "light") {
      document.documentElement.classList.remove("dark")
    } else {
      window.matchMedia('(prefers-color-scheme: dark)')
    }
  }

  return (
    <li onClick={handleChangeTheme} className="cursor-pointer">
      {iconsTheme[theme].icon}
    </li>
  )
}

export default ColorTheme